...
export default function BookCard({ book }) {
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            await axios.put(`/books/${book._id}`, { credits });
    ...
    return (
        <div className="group relative overflow-hidden rounded-xl bg-card border border-border shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-primary/30 h-full flex flex-col">
            <Link to={`/books/${book._id}`} className="aspect-[2/3] w-full overflow-hidden bg-muted relative block">
                <img
            ...
            <Link to={`/books/${book._id}`} className="flex-grow p-3 flex flex-col">
                ...
                <div className="mt-auto flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        {isEditing ? (
                            ...
                            <span className="text-sm font-bold text-primary flex items-center gap-1">
                                ₹{credits} 
                                {(user?.role === 'admin' || user?._id === (book.sellerId?._id || book.sellerId)) && (
                                    <button 
                    ...
                    {(user?.role === 'admin' || user?._id === book.sellerId?._id) && (
                         <button 
